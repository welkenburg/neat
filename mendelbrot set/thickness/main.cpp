...
*/
const char* path = { "thickness.bmp" };
const int width = 480;
const int height = 360;
char imageBuffer[height * (width * 3 + width % 4)];


long double centerPoint[2] = { -1,0 };
float zoom = 2;
int maxValue = 100;
	...
	for (int j = 0; j < height; j++) {
		for (int i = 0; i < width; i++) {
			...
			for (int n = 0; n < mendelbrot.maxInteration; n++) {
				...
				derivative = derivative * mendelbrot.two * z + mendelbrot.one;
				z = z * z + c;
			}
			if (in) {
				imageBuffer[j * (width * 3 + width % 4) + i * 3 + 0] = mendelbrot.setColor[0];
				imageBuffer[j * (width * 3 + width % 4) + i * 3 + 1] = mendelbrot.setColor[1];
				imageBuffer[j * (width * 3 + width % 4) + i * 3 + 2] = mendelbrot.setColor[2];
			}
			else {
				long double sqrModulusZ = mendelbrot.complexAbsolute(z);
				long double sqrModulusD = mendelbrot.complexAbsolute(boundingCoef * derivative);
				if (sqrModulusZ * log(sqrModulusZ) * log(sqrModulusZ) < sqrModulusD) {
					...
					char* colors = mendelbrot.basic(finalN);
