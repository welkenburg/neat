...
int main() {
	...
	for (int j = 0; j < height; j++) {
		for (int i = 0; i < width; i++) {
			...
			int n = 0;
			while (mendelbrot.complexAbsolute(z) < maxValue && n < mendelbrot.maxInteration) {
				z = z * z + c;
				n++;
			}
			char* colors = mendelbrot.potential(n, mendelbrot.complexAbsolute(z));
